      ...
      try {
        ...
        if (task.autofix) {
          ...
          try {
            ...
            results.push({ task: task.name, status: 'FAILED', blocking: task.blocking });
          }
        } else {
          results.push({ task: task.name, status: 'FAILED', blocking: task.blocking });
        }
        ...
        if (task.blocking) {
          console.log(`🛑 Workflow stopped due to blocking failure in: ${task.name}`);
          break;
        }
      }
    }
  ...
  switch (command) {
    ...
    case 'deployment':
      workflows.executeWorkflow(command).then(result => {
        ...
        console.log(`Fixed: ${result.results.filter(r => r.status === 'FIXED').length}`);
    ...
    case 'generate':
      ...
      if (workflowType) {
        const workflow = workflows[`create${workflowType.charAt(0).toUpperCase() + workflowType.slice(1)}Workflow`]();
        console.log(JSON.stringify(workflow, null, 2));
      } else {
        console.log('Usage: node task-workflows.js generate [preEdit|postEdit|ui|deployment]');
      }
      break;
    default:
      console.log('Usage: node task-workflows.js [pre-edit|post-edit|ui|deployment|generate]');
  }
}
