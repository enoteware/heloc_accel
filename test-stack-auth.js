...
class StackAuthTester {
  ...
  async testPage(url, pageName) {
    ...
    try {
      ...
      const screenshotPath = path.join(
        this.screenshotDir,
        `${pageName.replace(/[^a-zA-Z0-9]/g, "-")}.png`,
      );
      ...
      for (const selector of stackAuthSelectors) {
        try {
          ...
          if (elements.length > 0) {
            console.log(
              `  🔗 Found ${elements.length} Stack Auth element(s) with selector: ${selector}`,
            );
  ...
  determinePageStatus(pageResult) {
    ...
    if (
      pageResult.elements.forms.length === 0 &&
      pageResult.url.includes("sign")
    ) {
  ...
  printPageSummary(pageResult) {
    ...
    console.log(
      `  🔗 Stack Auth elements: ${pageResult.elements.stackAuthElements.length}`,
    );
  ...
  async generateReport() {
    ...
    Object.entries(this.results.pages).forEach(([name, page]) => {
      ...
      console.log(
        `  Stack Auth elements: ${page.elements.stackAuthElements.length}`,
      );
    ...
    Object.values(this.results.pages).forEach((page) => {
      if (page.status === "failed") {
        recommendations.push(
          `${page.name}: Page failed to load - check server status and URL routing`,
        );
      }

      if (page.url.includes("sign") && page.elements.forms.length === 0) {
        recommendations.push(
          `${page.name}: No forms found on authentication page - verify Stack Auth integration`,
        );
      }

      if (page.elements.stackAuthElements.length === 0) {
        recommendations.push(
          `${page.name}: No Stack Auth elements detected - verify component integration`,
        );
      }

      if (page.errors.some((e) => e.type === "console-error")) {
        recommendations.push(
          `${page.name}: Console errors detected - check browser console for details`,
        );
      }
    });
...
async function runStackAuthTests() {
  ...
  try {
    ...
    const pages = [
      ...
      {
        url: "http://localhost:3002/en/stack-auth-test",
        name: "Stack Auth Test Page",
      },
    ];
