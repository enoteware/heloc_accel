...
const problematicImports = [
  { import: '@/components/DemoAccountsInfo', file: 'src/components/DemoAccountsInfo.tsx' },
  { import: '@/components/CalculatorForm', file: 'src/components/CalculatorForm.tsx' },
...
if (fs.existsSync(tsconfigPath)) {
  ...
  console.log(`   @/* mapping: ${JSON.stringify(tsconfig.compilerOptions?.paths?.['@/*'] || 'not set')}`)
  console.log('')
}
...
if (fs.existsSync(srcComponents)) {
  ...
  if (componentFiles.length > 5) console.log(`     ... and ${componentFiles.length - 5} more`)
}
...
if (fs.existsSync(srcLib)) {
  ...
  if (libFiles.length > 5) console.log(`     ... and ${libFiles.length - 5} more`)
}
