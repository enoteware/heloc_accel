...
async function batchTranslate(strings, targetLang = "es", batchSize = 10) {
  ...
  console.log(
    `🔄 Translating ${strings.length} strings in ${batches.length} batches...`,
  );
...
async function main() {
  try {
    // Load user strings
    const userStrings = JSON.parse(
      fs.readFileSync("./user-strings.json", "utf8"),
    );

    console.log(
      `🌍 Starting free translation of ${userStrings.length} strings to Spanish`,
    );
    ...
    console.log(
      `🤖 Auto-translating ${needTranslation.length} remaining strings...`,
    );
    ...
    const spanishMessages = {
      ...
      calculator: {
        ...
        subtitle:
          "Compare estrategias de pago hipotecario tradicional vs aceleración HELOC",
        ...
        propertyValue:
          translations["Property Value"] || "Valor de la Propiedad",
      ...
      results: {
        ...
        helocAcceleration:
          translations["HELOC Acceleration"] || "Aceleración HELOC",
      ...
      validation: {
        required:
          translations["This field is required"] || "Este campo es obligatorio",
        positive:
          translations["Must be positive number"] ||
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(spanishMessages, null, 2),
    );
    ...
    console.log(`\n📊 Translation Summary:`);
    console.log(
      `   Manual translations: ${Object.keys(manualTranslations).length}`,
    );
