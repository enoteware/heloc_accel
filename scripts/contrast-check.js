...
const DANGEROUS_PATTERNS = [
  ...
  {
    pattern: /\bbg-white\b.*?\btext-white\b/g,
    severity: "error",
    message: "White text on white background",
  },
  {
    pattern: /\bbg-neutral-50\b.*?\btext-white\b/g,
    severity: "error",
    message: "White text on light background",
  },
  {
    pattern: /\bbg-neutral-100\b.*?\btext-neutral-100\b/g,
    severity: "error",
    message: "Same light colors",
  },
  {
    pattern: /\bbg-neutral-800\b.*?\btext-neutral-800\b/g,
    severity: "error",
    message: "Same dark colors",
  },
  {
    pattern: /\bbg-neutral-900\b.*?\btext-neutral-900\b/g,
    severity: "error",
    message: "Same dark colors",
  },
  {
    pattern: /\bbg-black\b.*?\btext-black\b/g,
    severity: "error",
    message: "Black text on black background",
  },

  // Potentially problematic combinations
  {
    pattern: /\bbg-yellow-[12]\d{2}\b.*?\btext-white\b/g,
    severity: "warning",
    message: "White text on yellow may have poor contrast",
  },
];
...
class ContrastChecker {
  ...
  checkFile(filePath) {
    try {
      ...
      if (
        filePath.includes("useContrastValidation.ts") ||
      ...
      while ((match = pattern.exec(content)) !== null) {
        ...
        if (this.containsPotentialIssue(match[0])) {
          this.issues.push({
            ...
            severity: "info",
            message:
              "Dynamic className - manual review needed for contrast safety",
            match: match[0],
            context: contextLine.trim(),
          });
        }
  ...
  generateReport() {
    ...
    if (errors.length > 0 || warnings.length > 0) {
      ...
      console.log(
        "4. Consider the useContrastValidation() hook for dynamic colors",
      );
      console.log("");
    }
