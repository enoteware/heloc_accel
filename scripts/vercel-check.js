...
console.log(`${colors.magenta}${colors.bright}▲ Running Vercel deployment checks...${colors.reset}\n`);
...
function checkVercelConfig() {
  ...
  if (!fs.existsSync(vercelJsonPath)) {
    ...
    console.log(`${colors.yellow}⚠️  No vercel.json found - using default settings${colors.reset}`);
  } else {
...
function checkNextConfig() {
  ...
  if (fs.existsSync(nextConfigPath)) {
    ...
    if (content.includes("output: 'standalone'") && !content.includes('//')) {
      errors.push("next.config.js has 'output: standalone' which is incompatible with Vercel");
      console.log(`${colors.red}❌ Standalone output mode is not compatible with Vercel${colors.reset}`);
    } else {
      console.log(`${colors.green}✅ No incompatible output mode${colors.reset}`);
    }
    
    // Check for custom server
    if (content.includes('server.js') || fs.existsSync(path.join(process.cwd(), 'server.js'))) {
      warnings.push('Custom server detected - not used in Vercel deployment');
      console.log(`${colors.yellow}⚠️  Custom server detected - will be ignored by Vercel${colors.reset}`);
    }
  }
...
function checkEnvironmentVariables() {
  ...
  if (fs.existsSync(envExamplePath)) {
    ...
    warnings.push('No .env.example found - ensure environment variables are documented');
  }
...
function checkFileSizes() {
  ...
  if (largeFiles.length > 0) {
    errors.push(`Found ${largeFiles.length} files exceeding Vercel's 50MB limit`);
    console.log(`${colors.red}❌ Large files detected:${colors.reset}`);
    largeFiles.forEach(file => {
      console.log(`  - ${file.path} (${file.size})`);
    });
  } else {
    console.log(`${colors.green}✅ All files within size limits${colors.reset}`);
  }
...
function checkVercelGotchas() {
  ...
  const srcFiles = execSync('find src -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" 2>/dev/null || true', { encoding: 'utf8' })
    .split('\n')
    .filter(Boolean);
  ...
  if (localhostCount > 0) {
    warnings.push(`Found ${localhostCount} files with hardcoded localhost URLs`);
    console.log(`${colors.yellow}⚠️  Found hardcoded localhost URLs in ${localhostCount} files${colors.reset}`);
  ...
  const clientComponents = execSync('grep -l "use client" src/**/*.tsx 2>/dev/null || true', { encoding: 'utf8' })
    .split('\n')
    .filter(Boolean);
  ...
  if (serverCodeInClient > 0) {
    errors.push(`Found ${serverCodeInClient} client components with server-only code`);
    console.log(`${colors.red}❌ Server code in client components (${serverCodeInClient} files)${colors.reset}`);
  } else {
    console.log(`${colors.green}✅ No server code in client components${colors.reset}`);
  }
...
function generateChecklist() {
  ...
  console.log('  [ ] Set all required environment variables in Vercel dashboard');
...
function main() {
  ...
  if (errors.length > 0) {
    console.log(`${colors.red}❌ VERCEL DEPLOYMENT CHECK FAILED${colors.reset}`);
