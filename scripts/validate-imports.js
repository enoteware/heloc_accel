...
function getAllSourceFiles(dir, fileList = []) {
  ...
  files.forEach(file => {
    ...
    if (stat.isDirectory()) {
      ...
      if (['node_modules', '.next', 'deployment', '.git', 'coverage'].includes(file)) {
        return;
      }
...
function validateImports() {
  ...
  if (fs.existsSync(tsConfigPath)) {
    try {
      ...
      console.warn(`${colors.yellow}Warning: Could not parse tsconfig.json${colors.reset}`);
    }
  }
  ...
  // Report results
  console.log(`Checked ${totalImports} imports across ${sourceFiles.length} files\n`);
  
  if (warnings.length > 0) {
    console.log(`${colors.yellow}⚠️  Warnings (${warnings.length}):${colors.reset}`);
    warnings.forEach(w => {
      console.log(`  ${w.file}:${w.line} - Cannot resolve '${w.module}' (${w.reason})`);
    });
    console.log('');
  }
  
  if (errors.length > 0) {
    console.log(`${colors.red}❌ Import Errors (${errors.length}):${colors.reset}`);
    errors.forEach(err => {
      console.log(`  ${colors.red}${err.file}:${err.line} - Cannot resolve '${err.module}'${colors.reset}`);
      console.log(`    Reason: ${err.reason}`);
    });
    console.log('');
    console.log(`${colors.red}Import validation failed with ${errors.length} error(s)${colors.reset}`);
    process.exit(1);
  } else {
    console.log(`${colors.green}✅ All imports validated successfully!${colors.reset}`);
  }
}
...
try {
  ...
  console.error(`${colors.red}Error during import validation:${colors.reset}`, error);
  process.exit(1);
}