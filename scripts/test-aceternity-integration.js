...
try {
  const indexPath = path.join(process.cwd(), 'src/components/design-system/index.ts');
...
try {
  const homePagePath = path.join(process.cwd(), 'src/app/[locale]/HomePageContent.tsx');
...
async function runServerTests() {
  ...
  await testEndpoint('/api/pexels/search?theme=money&per_page=1', 'Pexels API endpoint');
  ...
  console.log('• Import components: import { MoneyAceternityCard } from "@/components/design-system"');
}
