...
console.log(
  `${colors.blue}${colors.bright}🚀 Running pre-deployment checks for HELOC Accelerator...${colors.reset}\n`,
);
...
function runCommand(name, command, critical = true) {
  ...
  try {
    ...
    console.log(
      `${colors.yellow}⚠️  Warning: Non-critical check failed, continuing...${colors.reset}\n`,
    );
    return true;
  }
}
...
function checkNodeVersion() {
  ...
  if (majorVersion < 18) {
    console.error(
      `${colors.red}❌ Node.js version ${nodeVersion} is too old. Required: >= 18.0.0${colors.reset}\n`,
    );
    return false;
  }

  console.log(
    `${colors.green}✅ Node.js version ${nodeVersion} is compatible${colors.reset}\n`,
  );
  return true;
}
...
function checkEnvironment() {
  console.log(
    `${colors.yellow}▶ Environment Configuration Check${colors.reset}`,
  );
  ...
  if (!fs.existsSync(envExample)) {
    console.log(
      `${colors.yellow}⚠️  Warning: .env.example not found${colors.reset}`,
    );
  }

  if (!fs.existsSync(envLocal)) {
    console.log(
      `${colors.yellow}⚠️  Warning: .env.local not found (using demo mode)${colors.reset}`,
    );
  }
...
const checks = [
  ...
  },
  {
    ...
    preCheck: () => {
      ...
      if (!fs.existsSync(validatorPath)) {
        console.log(
          `${colors.yellow}  Creating import validator...${colors.reset}`,
        );
        return false;
      }
      return true;
  ...
  },
  {
    ...
    critical: true,
    preCheck: () => {
      const vercelCheckPath = path.join(
        process.cwd(),
        "scripts",
        "vercel-check.js",
      );
...
async function runChecks() {
  ...
  if (failed) {
    ...
    console.log(
      `${colors.green}✅ ALL PRE-DEPLOYMENT CHECKS PASSED!${colors.reset}`,
    );
    console.log(
...
process.on("unhandledRejection", (error) => {
  console.error(
    `${colors.red}Unexpected error during pre-deployment checks:${colors.reset}`,
    error,
  );
  process.exit(1);
});
