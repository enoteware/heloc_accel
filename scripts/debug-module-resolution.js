...
const failingImports = [
  { import: '@/components/DemoAccountsInfo', expectedFile: 'src/components/DemoAccountsInfo.tsx' },
  { import: '@/components/CalculatorForm', expectedFile: 'src/components/CalculatorForm.tsx' },
...
function generateRecommendations() {
  ...
  const missingFiles = failingImports.filter(({ expectedFile }) => !checkFile(expectedFile).exists)
  if (missingFiles.length > 0) {
    recommendations.push('❌ Missing files detected - restore from git or recreate')
  }
  ...
  if (recommendations.length === 0) {
    recommendations.push('✅ All basic checks passed - issue may be Vercel-specific')
    recommendations.push('🔄 Try: Clear Vercel build cache via dashboard')
    recommendations.push('🔄 Try: Different Next.js version')
    recommendations.push('🔄 Try: Remove .next and node_modules, fresh install')
  }
