...
export function useContrastValidation(): UseContrastValidationReturn {
  const validateCombination = useCallback(
    (background: string, textColor: string): ContrastValidationResult => {
      const result = validateContrast(textColor, background);
      const tailwindResult = validateTailwindCombination(
  ...
  const getSafeColors = useCallback(
    <T extends AllColors>(backgroundColor: T): SafeTextColor<T>[] => {
  ...
  const assertSafe = useCallback(
    (background: string, textColor: string, context?: string): void => {
    ...
    (props: {
      ...
      if (backgroundColor && textColor) {
        try {
          ...
          console.error(
            `[${componentName}] Contrast validation failed:`,
            error,
          );
        }
      }
      ...
      if (className) {
        ...
        if (!result.isValid) {
          console.warn(
            `[${componentName}] Potential contrast issues in className:`,
            {
 ...
 */
export function useContrastDebugger(
  enabled: boolean = process.env.NODE_ENV === "development",
) {
      ...
      dangerousCombinations: [
        {
          bg: "bg-white",
          text: "text-white",
          issue: "White on white - completely invisible",
        },
        {
          bg: "bg-neutral-50",
          text: "text-white",
          issue: "White on light gray - very poor contrast",
        },
        {
          bg: "bg-neutral-100",
        ...
        },
        {
          ...
          issue: "Same dark color - invisible",
        },
        {
          bg: "bg-black",
          text: "text-black",
          issue: "Black on black - completely invisible",
        },
      ],
