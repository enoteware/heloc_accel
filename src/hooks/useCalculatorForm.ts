...
import { FORM_VALIDATION_RULES, validateCrossFields } from '@/lib/form-validation'
...
export function useCalculatorForm(defaultValues?: Partial<CalculatorFormData>) {
  ...
  const watchedValues = useWatch({
    ...
    name: ['monthlyIncome', 'monthlyExpenses', 'monthlyPayment', 'currentMortgageBalance', 'helocLimit']
  })

  const [monthlyIncome, monthlyExpenses, monthlyPayment, mortgageBalance, helocLimit] = watchedValues
  ...
  useEffect(() => {
    ...
    if (income > 0 && expenses >= 0 && payment > 0) {
      ...
      setValue('monthlyDiscretionaryIncome', formatted, { shouldValidate: true })
    }
  ...
  const transformToCalculationInput = (data: CalculatorFormData): CalculationInput => {
    ...
    return {
      ...
      heloc_interest_rate: data.helocInterestRate ? parseNumber(data.helocInterestRate) / 100 : undefined,
      monthly_discretionary_income: parseNumber(data.monthlyDiscretionaryIncome)
    }
  }

  // Enhanced submit handler
  const onSubmit = (callback: (data: CalculationInput) => void | Promise<void>) => {
  ...
  const validationRules = {
    ...
    monthlyDiscretionaryIncome: FORM_VALIDATION_RULES.monthlyDiscretionaryIncome
  }
