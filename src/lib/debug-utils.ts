...
class DebugLogger {
  ...
  constructor() {
    ...
    this.isEnabled = 
      ...
      typeof window !== 'undefined' && window.localStorage?.getItem('debug-mode') === 'true'
  }

  log(level: DebugLog['level'], category: DebugLog['category'], message: string, data?: any, error?: Error) {
  ...
  getLogs(category?: DebugLog['category'], level?: DebugLog['level']): DebugLog[] {
...
export function debugLTVCalculation(
  ...
  if (loanAmount == null || propertyValue == null) {
    ...
    errors.push(`Missing required inputs: ${missingFields.join(' and ')} is null/undefined`)
    ...
    if (isNaN(loan) || isNaN(value)) {
      ...
      errors.push('Invalid values: loan amount negative or property value zero/negative')
  ...
  const logLevel = (loanAmount == null || propertyValue == null) ? 'warn' : 'error'
...
export function debugFormValidation(
  ...
  debugLogger.log(level, 'validation', `Form validation: ${formName}.${fieldName}`, debugInfo)

  return debugInfo
}

export function debugCalculationError(error: Error, context: any) {
  debugLogger.log('error', 'calculation', `Calculation error: ${error.message}`, context, error)
}
