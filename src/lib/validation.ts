...
export const VALIDATION_RULES = {
  mortgageBalance: {
    ...
    max: 10000000,
    message:
      "Mortgage balance must be between $1,000 and $10,000,000. Please check your latest mortgage statement for the principal balance.",
  },
  interestRate: {
    min: 0.001, // 0.1%
    max: 0.3, // 30%
    message:
      "Interest rate must be between 0.1% and 30%. Enter as a percentage (e.g., 6.5 for 6.5%).",
  },
  termMonths: {
    min: 12, // 1 year
    max: 480, // 40 years
    message:
      "Loan term must be between 1 and 40 years (12-480 months). Check your mortgage documents for remaining term.",
  },
  monthlyPayment: {
    min: 100,
    max: 100000,
    message:
      "Monthly payment must be between $100 and $100,000. This should be your principal and interest payment only.",
  },
  helocLimit: {
    min: 1000,
    max: 5000000,
    message:
      "HELOC limit must be between $1,000 and $5,000,000. This is your maximum approved credit line.",
  },
  income: {
    min: 1000,
    max: 1000000,
    message:
      "Monthly income must be between $1,000 and $1,000,000. Include all sources of regular monthly income.",
  },
  expenses: {
    min: 0,
    max: 500000,
    message:
      "Monthly expenses must be between $0 and $500,000. Include all regular monthly bills except your mortgage payment.",
  },
  propertyValue: {
    min: 10000,
    max: 50000000,
  ...
  pmiMonthly: {
    ...
    max: 5000,
    message:
      "MIP/PMI must be between $0 and $5,000 per month. This is required if your loan-to-value ratio exceeds 80%.",
  },
};
...
export interface CalculatorValidationInput
  extends MortgageValidationInput,
    Partial<HELOCValidationInput>,
...
export function validatePercentage(
  ...
  if (decimalValue < 0.001) {
    errors.push({
      field: fieldName,
      message:
        "Interest rate must be at least 0.1%. Please enter as a percentage (e.g., 6.5 for 6.5%).",
    });
  } else if (decimalValue > 0.3) {
    errors.push({
      field: fieldName,
      message:
        "Interest rate cannot exceed 30%. Please verify and enter as a percentage (e.g., 6.5 for 6.5%).",
    });
  }
...
export function validateMortgageInputs(
  ...
  if (input.currentMortgageBalance <= 0) {
    errors.push({
      ...
      message:
        "Mortgage balance must be greater than $0. Please enter your current principal balance.",
    });
  } else {
    errors.push(
      ...validateNumber(
  ...
  if (input.remainingTermMonths < 1) {
    errors.push({
      ...
      message:
        "Remaining term must be at least 1 month. Please check your mortgage statement for the exact number of months remaining.",
    });
  } else if (input.remainingTermMonths > 600) {
    errors.push({
      field: "remainingTermMonths",
      message:
        "Remaining term cannot exceed 600 months (50 years). Please verify the number of months remaining on your mortgage.",
    });
  }
    ...
    errors.push(
      ...validateNumber(
        ...
        "propertyTaxMonthly",
        {
          min: 0,
          max: 50000,
          message: "Property tax must be between $0 and $50,000",
        },
        false,
      ),
    );
    ...
    errors.push(
      ...validateNumber(
        ...
        "insuranceMonthly",
        {
          min: 0,
          max: 10000,
          message: "Insurance must be between $0 and $10,000",
        },
        false,
      ),
    );
  ...
  if (
    ...
    input.currentMortgageBalance !== undefined
  ) {
    try {
      ...
      if (
        isMIPRequired(ltvRatio) &&
        (input.pmiMonthly === undefined || input.pmiMonthly === 0)
      ) {
      ...
      errorMonitor.reportCalculationError(
        "ltv",
        {
          loanAmount: input.currentMortgageBalance,
...
export function validateHELOCInputs(
  input: HELOCValidationInput,
): ValidationResult {
  const errors: ValidationError[] = [];
  ...
  if (input.helocLimit <= 0) {
    errors.push({
      field: "helocLimit",
      message:
        "HELOC limit must be greater than $0. Please enter your approved HELOC credit limit.",
    });
  } else {
    errors.push(
      ...validateNumber(
  ...
  errors.push(
    ...validatePercentage(input.helocInterestRate, "helocInterestRate"),
  );
...
export function validateIncomeInputs(
  ...
  errors.push(
    ...validateNumber(
      ...
      "monthlyDiscretionaryIncome",
      {
        min: 0,
        max: 500000,
        message: "Discretionary income must be between $0 and $500,000",
      },
    ),
  );
  ...
  if (
    !isNaN(input.monthlyDiscretionaryIncome) &&
    !isNaN(input.monthlyNetIncome) &&
    !isNaN(input.monthlyExpenses)
  ) {
    const calculatedDiscretionary =
      input.monthlyNetIncome - input.monthlyExpenses;
    const tolerance = 50; // Allow $50 tolerance for rounding

    if (
      Math.abs(input.monthlyDiscretionaryIncome - calculatedDiscretionary) >
      tolerance
    ) {
      errors.push({
...
export function validateCalculatorInputs(
  input: CalculatorValidationInput,
): ValidationResult {
  const errors: ValidationError[] = [];
  ...
  if (
    ...
    input.scenarioName.trim().length === 0
  ) {
    errors.push({
      field: "scenarioName",
      message:
        'Please provide a name for this scenario (e.g., "Current Mortgage" or "HELOC Strategy")',
    });
  }
...
export function sanitizeCalculatorInputs(
  ...
  return {
    ...
    propertyValue: input.propertyValue
      ? parseFloat(input.propertyValue)
      : undefined,
    propertyTaxMonthly: input.propertyTaxMonthly
      ? parseFloat(input.propertyTaxMonthly)
      : undefined,
    insuranceMonthly: input.insuranceMonthly
      ? parseFloat(input.insuranceMonthly)
      : undefined,
    hoaFeesMonthly: input.hoaFeesMonthly
      ? parseFloat(input.hoaFeesMonthly)
      : undefined,
    ...
    helocAvailableCredit: input.helocAvailableCredit
      ? parseFloat(input.helocAvailableCredit)
      : undefined,
    ...
    monthlyDiscretionaryIncome:
      parseFloat(input.monthlyDiscretionaryIncome) || 0,
