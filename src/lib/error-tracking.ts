...
class ErrorTracker {
   ...
   */
  captureMessage(message: string, level: 'debug' | 'info' | 'warning' | 'error' = 'info', context?: Partial<ErrorContext>): void {
    this.addBreadcrumb({
      category: 'message',
      message,
      level,
 ...
 * API error handler
 */
export function captureAPIError(error: Error, request: Request, context?: Partial<ErrorContext>): string {
  return errorTracker.captureError(error, {
    url: request.url,
    action: 'api_request',
    metadata: {
 ...
 */
export function captureDatabaseError(error: Error, query: string, params?: any[]): string {
  return errorTracker.captureError(error, {
    component: 'database',
    action: 'query',
    metadata: {
