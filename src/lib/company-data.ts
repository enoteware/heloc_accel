...
const DEMO_COMPANY_SETTINGS: CompanySettings = {
  ...
  companyDescription: 'Your trusted partner in mortgage acceleration strategies. We help homeowners save thousands in interest and pay off their mortgages years early.',
  primaryColor: '#2563eb',
...
const DEMO_AGENTS: Agent[] = [
  {
    ...
    specialties: ['HELOC', 'Mortgage Refinancing', 'Debt Consolidation', 'First-Time Buyers'],
    yearsExperience: 15,
    isActive: true,
    displayOrder: 1
  },
...
export function updateDemoCompanySettings(settings: Partial<CompanySettings>): CompanySettings {
  if (typeof window === 'undefined') throw new Error('Cannot update settings on server side')
...
export function getDemoUserAgent(userId: string): Agent | null {
  ...
  if (!assignment) {
    ...
    localStorage.setItem(STORAGE_KEYS.USER_ASSIGNMENTS, JSON.stringify(assignments))
...
export function getAgentContactInfo(agent: Agent): {
  ...
  return {
    ...
    mobile: agent.mobilePhone ? formatPhoneNumber(agent.mobilePhone) : undefined
  }
}