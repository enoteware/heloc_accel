...
describe("Calculator Forms - LTV/PMI Integration Tests", () => {
  ...
  describe("FastCalculatorForm", () => {
    ...
    it("should display LTV analysis when both loan amount and property value are provided", async () => {
      ...
      const mortgageBalanceInput = screen.getByLabelText(
        /current mortgage balance/i,
      );
    ...
    it("should handle invalid inputs gracefully", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText(/loan-to-value analysis/i),
        ...
        for (const { component: FormComponent, props } of forms) {
          ...
          await waitFor(() => {
            if (scenario.expectedPMIRequired) {
              expect(
                screen.getByText(/mip\/pmi.*required/i),
              ).toBeInTheDocument();
            } else {
              expect(
                screen.queryByText(/mip\/pmi.*required/i),
              ).not.toBeInTheDocument();
            }
          });
  ...
  describe("Performance Tests", () => {
    it("should handle rapid input changes without performance issues", async () => {
      ...
      for (let i = 0; i < 10; i++) {
        fireEvent.change(mortgageInput, {
          target: { value: `${400000 + i * 1000}` },
        });
        fireEvent.change(propertyInput, {
          target: { value: `${500000 + i * 1000}` },
        });
      }
