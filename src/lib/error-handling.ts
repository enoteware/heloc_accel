...
export function validateCalculationInputs(inputs: any): void {
  ...
  if (inputs.annualInterestRate > 0.5) {
    // 50%
    throw new CalculationError(
...
export function safeDivide(
  ...
  if (denominator === 0) {
    throw new MathematicalError("Division by zero", "division", {
      numerator,
      denominator,
    });
  }
  ...
  if (!isFinite(numerator) || !isFinite(denominator)) {
    throw new MathematicalError("Invalid numbers in division", "division", {
      numerator,
      denominator,
    });
  }
...
export function safePower(base: number, exponent: number): number {
  if (!isFinite(base) || !isFinite(exponent)) {
    throw new MathematicalError("Invalid numbers in power operation", "power", {
      base,
      exponent,
    });
  }
...
export function recoverFromCalculationError(
  error: Error,
  fallbackValue?: any,
): any {
