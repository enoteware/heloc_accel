  ...
  currentMortgageBalance: {
    ...
    validate: {
      realistic: (value: string) => {
        ...
        if (numValue < 50000)
          return "This seems low for a mortgage balance. Please verify.";
        if (numValue > 2000000)
          return "This is a very high mortgage balance. Please verify.";
        return true;
      },
  ...
  remainingTermMonths: {
    ...
    validate: {
      wholeNumber: (value: string) => {
        const numValue = parseFloat(value);
        return (
          Number.isInteger(numValue) || "Term must be a whole number of months"
        );
      },
    },
  },
    ...
    validate: {
      reasonable: (value: string, formValues: any) => {
        ...
        const balance = parseFloat(
          formValues.currentMortgageBalance?.replace(/[,$]/g, "") || "0",
        );
  ...
  helocLimit: {
    ...
    validate: {
      reasonable: (value: string, formValues: any) => {
        ...
        const mortgageBalance = parseFloat(
          formValues.currentMortgageBalance?.replace(/[,$]/g, "") || "0",
        );
    ...
    validate: {
      reasonable: (value: string, formValues: any) => {
        ...
        const income = parseFloat(
          formValues.monthlyIncome?.replace(/[,$]/g, "") || "0",
        );
    ...
    validate: {
      calculated: (value: string, formValues: any) => {
        ...
        const expenses = parseFloat(
          formValues.monthlyExpenses?.replace(/[,$]/g, "") || "0",
        );
        const payment = parseFloat(
          formValues.monthlyPayment?.replace(/[,$]/g, "") || "0",
        );
    ...
    validate: {
      ltvBased: (value: string, formValues: any) => {
        ...
        const propertyValue = parseFloat(
          formValues.propertyValue?.replace(/[,$]/g, "") || "0",
        );
        ...
        if (mortgageBalance > 0 && propertyValue > 0) {
          try {
            ...
            console.warn(
              "LTV calculation failed during form validation:",
              error,
            );
          }
        }
...
export function validateCrossFields(formValues: any) {
  ...
  const discretionary = parseFloat(
    formValues.monthlyDiscretionaryIncome?.replace(/[,$]/g, "") || "0",
  );
  ...
  if (income > 0 && totalDebt / income > 0.5) {
    errors.monthlyExpenses =
      "Total debt payments exceed 50% of income - this may limit HELOC options";
  }
