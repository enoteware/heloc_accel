...
class ErrorMonitor {
  ...
  reportError(
    errorData: Omit<ErrorReport, "id" | "timestamp" | "userAgent" | "url">,
  ): string {
    ...
    if (process.env.NODE_ENV === "development") {
      console.error(
        `[ERROR MONITOR] ${error.type.toUpperCase()}: ${error.message}`,
        error,
      );
    }
...
if (typeof window !== "undefined") {
  (window as any).errorMonitor = {
    getErrors: (type?: string, severity?: string) =>
      errorMonitor.getErrors(type as any, severity as any),
