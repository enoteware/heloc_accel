...
class Logger {
  ...
  logAuthentication(event: 'login' | 'logout' | 'register' | 'failed_login', userId?: string, ip?: string): void {
...
export function createRequestLogger() {
  return {
    ...
    end: (startTime: number, method: string, url: string, statusCode: number, userId?: string) => {
...
export class PerformanceMonitor {
  ...
  static measure<T>(operation: string, fn: () => T, metadata?: Record<string, any>): T {
...
export function logHealthCheck(service: string, status: 'healthy' | 'unhealthy', details?: Record<string, any>): void {
  if (status === 'healthy') {
