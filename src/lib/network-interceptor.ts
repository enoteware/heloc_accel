...
class NetworkInterceptor {
  ...
  private install() {
    ...
    window.fetch = async function(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
      ...
      if (init?.body) {
        try {
          requestLog.requestBody = typeof init.body === 'string' 
      ...
      logDebug('Network', `${method} ${url}`, { requestId, body: requestLog.requestBody })
      ...
      try {
        ...
        logInfo('Network', `${method} ${url} - ${response.status} (${duration}ms)`, {
          requestId,
  ...
  public getLogs(filter?: { method?: string; status?: number; url?: string }): NetworkLog[] {
