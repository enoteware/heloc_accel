...
export function sanitizeCalculatorInput(input: any): SanitizedCalculatorInput {
  return {
    ...
    propertyValue: input.propertyValue
      ? sanitizeNumber(input.propertyValue)
      : undefined,
    propertyTaxMonthly: input.propertyTaxMonthly
      ? sanitizeNumber(input.propertyTaxMonthly)
      : undefined,
    insuranceMonthly: input.insuranceMonthly
      ? sanitizeNumber(input.insuranceMonthly)
      : undefined,
    hoaFeesMonthly: input.hoaFeesMonthly
      ? sanitizeNumber(input.hoaFeesMonthly)
      : undefined,
    helocLimit: input.helocLimit ? sanitizeNumber(input.helocLimit) : undefined,
    helocInterestRate: input.helocInterestRate
      ? sanitizeNumber(input.helocInterestRate)
      : undefined,
    helocAvailableCredit: input.helocAvailableCredit
      ? sanitizeNumber(input.helocAvailableCredit)
      : undefined,
    ...
    monthlyDiscretionaryIncome: sanitizeNumber(
...
export function sanitizeRateLimitKey(key: string): string {
  return key.replace(/[^a-zA-Z0-9:.-]/g, "_").slice(0, 100);
}
