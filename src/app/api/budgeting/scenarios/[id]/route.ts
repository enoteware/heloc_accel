...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      logError("API:BudgetScenario", "Stack Auth error", error);
    ...
    try {
      ...
      logInfo("API:BudgetScenario", "Budget scenario retrieved successfully", {
...
export async function PUT(
  ...
  try {
    ...
    try {
      ...
      logError("API:BudgetScenario", "Stack Auth error in PUT", error);
      return NextResponse.json(
        { error: "Authentication error" },
        { status: 401 },
      );
    ...
    if (
      ...
      if (!validation.isValid) {
        logError("API:BudgetScenario", "Validation failed", validation.errors);
        return NextResponse.json(
          {
...
export async function DELETE(
  ...
  try {
    ...
    try {
      ...
      logError("API:BudgetScenario", "Stack Auth error in DELETE", error);
      return NextResponse.json(
        { error: "Authentication error" },
        { status: 401 },
      );
