...
describe("Mortgage Calculations", () => {
  describe("calculateMonthlyPayment", () => {
    test("should calculate correct payment for standard 30-year mortgage", () => {
      ...
      expect(payment).toBeCloseTo(1896.2, 2);
    });
  ...
  describe("generateAmortizationSchedule", () => {
    ...
    test("should handle partial balance (refinanced mortgage)", () => {
      ...
      expect(
        schedule.schedule[schedule.schedule.length - 1].endingBalance,
      ).toBeCloseTo(0, 2);
    ...
    test("should calculate cumulative totals correctly", () => {
      ...
      expect(lastPayment.cumulativeInterest).toBeCloseTo(
...
describe("HELOC Calculations", () => {
  describe("calculateHELOCAcceleration", () => {
    ...
    test("should calculate average HELOC balance correctly", () => {
      ...
      expect(result.averageHelocBalance).toBeLessThanOrEqual(
        result.maxHelocUsed,
      );
  ...
  describe("compareStrategies", () => {
    test("should show HELOC benefits when advantageous", () => {
      ...
      expect(comparison.heloc.payoffMonths).toBeLessThanOrEqual(
        comparison.traditional.payoffMonths,
      );
    });
    ...
    test("should calculate percentage savings correctly", () => {
      ...
      if (comparison.comparison.interestSaved > 0) {
        const expectedPercentage =
          (comparison.comparison.interestSaved /
            comparison.traditional.totalInterest) *
          100;
        expect(comparison.comparison.percentageInterestSaved).toBeCloseTo(
          expectedPercentage,
          1,
        );
      }
  ...
  describe("Mathematical Edge Cases", () => {
    ...
    test("should handle near-zero balances correctly", () => {
      const input: MortgageInput = {
        ...
        annualInterestRate: 0.06,
        termInMonths: 360,
        currentBalance: 0.5, // Very small remaining balance
      };
