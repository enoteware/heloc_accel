...
export const HamburgerButton: React.FC<HamburgerButtonProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Hamburger icon with animated transformation using semantic tokens */}
      <div className="relative w-6 h-6">
        {/* Top line */}
        <span
          className={cn(
            "absolute left-0 top-1 w-6 h-0.5 rounded-full transition-all duration-300 ease-in-out",
            "bg-current", // Uses the current text color (semantic token)
        ...
        <span
          className={cn(
            "absolute left-0 top-4 w-6 h-0.5 rounded-full transition-all duration-300 ease-in-out",
            "bg-current", // Uses the current text color (semantic token)
            isOpen ? "-rotate-45 -translate-y-2" : "rotate-0 translate-y-0",
          )}
        />
