...
export default function ErrorDisplay({ error, errorDetails, onDismiss, onRetry }: ErrorDisplayProps) {
  const getErrorIcon = () => {
  ...
  return (
    <div className={`bg-${color}-50 border-2 border-${color}-200 rounded-lg p-4 mb-6`}>
      <div className="flex">
        ...
        <div className="ml-3 flex-1">
          ...
          {errorDetails?.field && (
            <div className={`mt-2 text-xs text-${color}-600`}>
              <span className="font-medium">Related field:</span> {errorDetails.field}
            </div>
          )}

          {process.env.NODE_ENV === 'development' && errorDetails?.technicalMessage && (
            <details className="mt-3">
              <summary className={`text-xs text-${color}-600 cursor-pointer hover:text-${color}-700`}>
                Technical details
              </summary>
              <pre className={`mt-1 text-xs text-${color}-600 overflow-x-auto`}>
