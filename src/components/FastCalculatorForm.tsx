...
export default function FastCalculatorForm({
  onSubmit,
  loading = false,
  initialData = {},
  validationErrors = [],
  ...
  >(() => ({
    ...
    remainingTermYears:
      initialData.remainingTermYears ||
      (initialData.remainingTermMonths
        ? Math.round(initialData.remainingTermMonths / 12)
        : 0),
    remainingTermMonths:
      initialData.remainingTermMonths ||
      (initialData.remainingTermYears
        ? initialData.remainingTermYears * 12
        : 0),
  ...
  const handleInputChange = useCallback(
    (
      ...
      debugLogger.log(
        "debug",
        "form",
        `FastCalculatorForm input change: ${field}`,
          ...
          const netIncome =
            field === "monthlyNetIncome"
              ? typeof processedValue === "number"
                ? processedValue
                : 0
              : typeof prev.monthlyNetIncome === "number"
                ? prev.monthlyNetIncome
                : 0;
          const expenses =
            field === "monthlyExpenses"
              ? typeof processedValue === "number"
                ? processedValue
                : 0
              : typeof prev.monthlyExpenses === "number"
                ? prev.monthlyExpenses
                : 0;

          if (netIncome > 0 && expenses >= 0) {
            newData.monthlyDiscretionaryIncome = Math.max(
              0,
              netIncome - expenses,
            );
          }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      const processedData: CalculatorValidationInput = {
        ...
        remainingTermMonths:
          Number(formData.remainingTermMonths) ||
          Number(formData.remainingTermYears) * 12 ||
          0,
        ...
        monthlyDiscretionaryIncome:
          Number(formData.monthlyDiscretionaryIncome) || 0,

        // Optional numeric fields - only include if they have values
        ...(formData.propertyValue &&
          Number(formData.propertyValue) > 0 && {
            propertyValue: Number(formData.propertyValue),
          }),
        ...(formData.propertyTaxMonthly &&
          Number(formData.propertyTaxMonthly) > 0 && {
            propertyTaxMonthly: Number(formData.propertyTaxMonthly),
          }),
        ...(formData.insuranceMonthly &&
          Number(formData.insuranceMonthly) > 0 && {
            insuranceMonthly: Number(formData.insuranceMonthly),
          }),
        ...(formData.hoaFeesMonthly &&
          Number(formData.hoaFeesMonthly) > 0 && {
            hoaFeesMonthly: Number(formData.hoaFeesMonthly),
          }),
        ...(formData.pmiMonthly &&
          Number(formData.pmiMonthly) > 0 && {
            pmiMonthly: Number(formData.pmiMonthly),
          }),
        ...(formData.helocAvailableCredit &&
          Number(formData.helocAvailableCredit) > 0 && {
            helocAvailableCredit: Number(formData.helocAvailableCredit),
          }),
      ...
      requiredFields.forEach((field) => {
        ...
        const value = processedData[
          field as keyof CalculatorValidationInput
        ] as number;
      ...
      if (
        processedData.helocLimit &&
        processedData.helocAvailableCredit &&
        processedData.helocAvailableCredit > processedData.helocLimit
      ) {
      ...
      if (
        processedData.helocLimit &&
        processedData.helocAvailableCredit &&
        processedData.helocLimit === processedData.helocAvailableCredit
      ) {
        newErrors.helocAvailableCredit =
      ...
      // Income validation
      if (
        processedData.monthlyNetIncome &&
        processedData.monthlyGrossIncome &&
      ...
      if (
        processedData.monthlyNetIncome &&
        processedData.monthlyExpenses &&
        processedData.monthlyDiscretionaryIncome
      ) {
        const calculatedDiscretionary =
          processedData.monthlyNetIncome - processedData.monthlyExpenses;
        const tolerance = 50;
        if (
          Math.abs(
            processedData.monthlyDiscretionaryIncome - calculatedDiscretionary,
          ) > tolerance
        ) {
