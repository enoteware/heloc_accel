...
export default function LiveResultsPanel({ results, loading = false, error = null, currentFormData = null }: LiveResultsPanelProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <h3 className="text-lg font-semibold text-green-900 mb-4">{tResults('potentialSavingsWithHeloc')}</h3>
          ...
          {loading ? (
            ...
            >
              <div>
                ...
                <p className="text-sm text-green-600">{tResults('totalInterestSaved')}</p>
              </div>
              <div className="grid grid-cols-2 gap-4 mt-4">
                <div>
                  <p className="text-xl font-semibold text-green-700">
                    {results.comparison.timeSavedYears.toFixed(1)} {tResults('years')}
                  </p>
                  <p className="text-xs text-green-600">{tResults('timeSaved')}</p>
                </div>
                <div>
                  <p className="text-xl font-semibold text-green-700">
                    {results.comparison.percentageInterestSaved.toFixed(0)}%
                  </p>
                  <p className="text-xs text-green-600">{tResults('interestReduction')}</p>
                </div>
              </div>
            </motion.div>
          ) : null}
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        >
          <h4 className="text-sm font-medium text-gray-600 mb-3">{tResults('traditionalMortgage')}</h4>
            ...
            {loading ? (
              ...
              >
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-500">{tResults('payoffTime')}</span>
                  <span className="font-semibold text-gray-900">{(results.traditional.payoffMonths / 12).toFixed(1)} {tResults('years')}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-500">{tResults('totalInterest')}</span>
                  <span className="font-semibold text-gray-900">{formatCurrency(results.traditional.totalInterest)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-500">{tResults('monthlyPayment')}</span>
        ...
        >
          <h4 className="text-sm font-medium text-blue-700 mb-3">{tResults('helocStrategy')}</h4>
          ...
          <AnimatePresence mode="wait">
            {loading ? (
              ...
              >
                <div className="flex justify-between items-center">
                  <span className="text-sm text-blue-600">{tResults('payoffTime')}</span>
                  <span className="font-semibold text-blue-700">{(results.heloc.payoffMonths / 12).toFixed(1)} {tResults('years')}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-blue-600">{tResults('totalInterest')}</span>
                  <span className="font-semibold text-blue-700">{formatCurrency(results.heloc.totalInterest)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-blue-600">{tResults('maxHelocUsed')}</span>
      ...
      {results && (
        ...
        >
          <h4 className="text-sm font-medium text-gray-700 mb-3">{tResults('quickAnalysis')}</h4>
          <div className="space-y-2 text-sm">
            <div className="flex items-center">
              ...
              <span className="text-gray-600">
                {tResults('payOffMortgageFaster', { years: results.comparison.timeSavedYears.toFixed(1) })}
              </span>
            </div>
            <div className="flex items-center">
              <svg className="h-4 w-4 text-green-500 mr-2" fill="currentColor" viewBox="0 0 20 20">
              ...
              <span className="text-gray-600">
                {tResults('saveInInterest', { amount: formatCurrency(results.comparison.interestSaved) })}
              </span>
            </div>
            <div className="flex items-center">
              ...
              <span className="text-gray-600">
                {tResults('usingHelocLimit', { amount: formatCurrency(results.heloc.maxHelocUsed) })}
              </span>
            </div>
          </div>
        </motion.div>
      ...
      {results && currentFormData && (
        ...
        >
          <button
            onClick={async () => {
              try {
                ...
                // Generate filename
                const filename = generateReportFilename(currentFormData.scenarioName)
          ...
          >
            ...
            <span>{tResults('downloadPdfReport')}</span>
          </button>
        </motion.div>
      )}
