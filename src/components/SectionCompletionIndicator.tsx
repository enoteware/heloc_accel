...
export const SectionCompletionIndicator: React.FC<
  SectionCompletionIndicatorProps
> = ({ formData, requiredFields, color, size = "md" }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      <div
        className={`inline-flex items-center justify-center rounded-full border-2 transition-all duration-300 ${sizeClass} ${
      ...
      >
        {isComplete ? (
          ...
          <div
            className={`rounded-full ${iconSizeClass} ${
              completedFields.length > 0
                ? colors.textComplete + " bg-current opacity-20"
                : ""
            }`}
          />
        )}
