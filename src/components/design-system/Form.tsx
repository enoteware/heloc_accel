...
export interface FormProps
  ...
  children:
    | React.ReactNode
    | ((form: UseFormValidationReturn) => React.ReactNode);
...
export function withFormField<T extends object>(
  ...
  const WrappedComponent = React.forwardRef<any, T & WithFormFieldProps>(
    (props, ref) => {
...
export const FormContext = React.createContext<UseFormValidationReturn | null>(
  null,
);
...
export const ConnectedField: React.FC<ConnectedFieldProps> = ({
  name,
  children,
}) => {
...
export const FormProgress: React.FC<FormProgressProps> = ({ className }) => {
  ...
  const completedFields = Object.entries(form.values).filter(([key, value]) => {
    return (
      value !== "" && value !== null && value !== undefined && !form.errors[key]
    );
  }).length;
