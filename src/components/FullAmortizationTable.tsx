...
export default function FullAmortizationTable({ 
  ...
  // Find key milestones
  const pmiDropOffMonth = amortizationData.find(row => row.ltvRatio <= 80)?.month
  const helocPaidOffMonth = amortizationData.findIndex((row, idx) => 
    idx > 0 && amortizationData[idx - 1].helocBalance > 0 && row.helocBalance === 0
  ) + 1
  ...
  const columnGroups = {
    ...
    heloc: ['usableHelocRoom', 'extraPaymentToMortgage', 'netChangeInHeloc', 'helocBalance', 'helocInterest'],
    ...
    <div className={cn('space-y-4', className)}>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            <Icon name={showAllColumns ? 'eye' : 'eye-off'} size="xs" className="mr-1 inline" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
          ...
          <p className="text-2xl font-bold text-blue-900">Month {schedule.length}</p>
          <p className="text-sm text-blue-700">
            Total saved: {formatCurrency(amortizationData[amortizationData.length - 1]?.totalInterestPaid || 0)}
          </p>
        </div>
        ...
        {pmiDropOffMonth && (
          <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
            <div className="flex items-center gap-2 mb-2">
              <Icon name="trending-down" size="sm" className="text-purple-600" />
              <h4 className="font-semibold text-purple-900">PMI Drops Off</h4>
            </div>
            <p className="text-2xl font-bold text-purple-900">Month {pmiDropOffMonth}</p>
            <p className="text-sm text-purple-700">
              LTV reaches 80%, saving {formatCurrency(inputs.pmiMonthly || 0)}/mo
            </p>
          </div>
        )}
        ...
        {helocPaidOffMonth > 0 && (
          <div className="bg-green-50 p-4 rounded-lg border border-green-200">
            ...
            <p className="text-2xl font-bold text-green-900">Month {helocPaidOffMonth}</p>
            <p className="text-sm text-green-700">
              All HELOC balance cleared
            </p>
          </div>
        )}
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead>
            <tr className="bg-gray-50">
              ...
              <th colSpan={3} className="px-3 py-2 text-center text-xs font-medium text-gray-700 uppercase tracking-wider border-b border-r border-gray-200 bg-gray-100">
                Mortgage
              </th>
              
              {/* Cash Flow Section */}
              {(showAllColumns || true) && (
                <th colSpan={3} className="px-3 py-2 text-center text-xs font-medium text-blue-700 uppercase tracking-wider border-b border-r border-gray-200 bg-blue-50">
                  Cash Flow
                </th>
              )}
              
              {/* HELOC Section */}
              <th colSpan={5} className="px-3 py-2 text-center text-xs font-medium text-secondary-700 uppercase tracking-wider border-b border-r border-gray-200 bg-secondary-50">
                HELOC Strategy
              </th>
              ...
              {showAllColumns && (
                <>
                  <th colSpan={2} className="px-3 py-2 text-center text-xs font-medium text-purple-700 uppercase tracking-wider border-b border-r border-gray-200 bg-purple-50">
                    PMI/LTV
                  </th>
                  <th colSpan={3} className="px-3 py-2 text-center text-xs font-medium text-green-700 uppercase tracking-wider border-b border-gray-200 bg-green-50">
                    Totals
                  </th>
                </>
              )}
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {amortizationData.map((row, index) => {
              ...
              const isMilestoneMonth = row.month === pmiDropOffMonth || row.month === helocPaidOffMonth
                ...
                <React.Fragment key={row.month}>
                  ...
                  >
                    ...
                    <td className={cn(
                      ...
                      row.extraPaymentToMortgage > 0 ? "bg-secondary-50 text-secondary-700" : "text-gray-400"
                    )}>
                      {formatCurrency(row.extraPaymentToMortgage)}
                    </td>
                    <td className="px-3 py-2 text-sm text-right text-gray-700 whitespace-nowrap">
                      <span className={cn(
                        row.netChangeInHeloc > 0 ? "text-red-600" : row.netChangeInHeloc < 0 ? "text-green-600" : "text-gray-400"
                      )}>
                        {row.netChangeInHeloc > 0 ? '+' : ''}{formatCurrency(Math.abs(row.netChangeInHeloc))}
                      </span>
                    </td>
                    <td className={cn(
                      "px-3 py-2 text-sm text-right whitespace-nowrap",
                      isHelocActive ? "font-medium text-blue-700" : "text-gray-400"
                    )}>
                    ...
                    {showAllColumns && (
                      <>
                        <td className={cn(
                          "px-3 py-2 text-sm text-right whitespace-nowrap",
                          isPMIActive && highlightPMI ? "bg-purple-50 text-purple-700 font-medium" : "text-gray-700"
                        )}>
                          {formatCurrency(row.pmiCost)}
                        </td>
                        <td className={cn(
                          "px-3 py-2 text-sm text-right border-r border-gray-200 whitespace-nowrap",
                          row.ltvRatio > 80 ? "text-red-600" : "text-green-600",
                          "font-medium"
                        )}>
                  ...
                  {isExpanded && (
                    <tr>
                      <td colSpan={showAllColumns ? 16 : 12} className="px-6 py-4 bg-blue-50 border-b border-blue-200">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                          <div>
                            ...
                            <div className="space-y-1">
                              <div className="flex justify-between">
                                ...
                                <span className="font-medium">{formatCurrency(row.interestPayment)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-600">Principal:</span>
                                <span className="font-medium">{formatCurrency(row.principalPayment)}</span>
                              </div>
                              {row.pmiCost > 0 && (
                                <div className="flex justify-between">
                                  <span className="text-gray-600">PMI:</span>
                                  <span className="font-medium text-purple-700">{formatCurrency(row.pmiCost)}</span>
                                </div>
                              )}
                            </div>
                          </div>
                          ...
                          <div>
                            <h5 className="font-semibold text-gray-900 mb-2">HELOC Details</h5>
                            <div className="space-y-1">
                              <div className="flex justify-between">
                                <span className="text-gray-600">Available:</span>
                                <span className="font-medium">{formatCurrency(row.usableHelocRoom)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-600">Used:</span>
                                <span className="font-medium">{formatCurrency(row.helocBalance)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-600">Interest:</span>
                                <span className="font-medium">{formatCurrency(row.helocInterest)}</span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div>
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className="font-medium">{formatCurrency(row.totalInterestPaid)}</span>
                              </div>
                            </div>
                          </div>
                        </div>
          ...
          <tfoot className="bg-gray-50">
            <tr className="font-semibold">
              ...
              <td colSpan={showAllColumns ? 15 : 11} className="px-3 py-3 text-sm text-right text-gray-900">
                <div className="flex justify-end items-center gap-6">
                  <span>Total Interest Paid:</span>
                  <span className="text-lg text-green-700">
                    {formatCurrency(amortizationData[amortizationData.length - 1]?.totalInterestPaid || 0)}
                  </span>
                  <span className="text-gray-500">|</span>
                  <span>Months to Payoff:</span>
                  <span className="text-lg text-blue-700">{schedule.length}</span>
                </div>
              </td>
            </tr>
          </tfoot>
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        <h4 className="text-sm font-semibold text-gray-900 mb-2">Column Guide</h4>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-secondary-50 border border-secondary-200 rounded"></div>
            <span className="text-gray-600">Extra Payment (HELOC Strategy)</span>
          </div>
          <div className="flex items-center gap-2">
