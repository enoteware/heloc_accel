...
test.describe('Complete Auth Debug Testing', () => {
  test('full authentication flow with interactive debug testing', async ({ page }) => {
    ...
    const userInfoSection = page.locator('h2:has-text("Current User"), h2:has-text("User Information")').first();
    const userExists = await userInfoSection.isVisible();
    console.log(`   User info section visible: ${userExists}`);
    
    if (userExists) {
      // Get user details
      const userId = await page.locator('text=/ID:.*enoteware/', 'text=/id:.*enoteware/').first().textContent();
      const userEmail = await page.locator('text=/Email:.*enoteware/', 'text=/email:.*enoteware/').first().textContent();
    ...
    if (authButtonExists) {
      ...
      const authResultsExpander = page.locator('summary:has-text("Auth Debug Results"), details:has-text("Auth Debug")').first();
      ...
      if (authResultsVisible) {
        ...
        console.log(`   Auth debug results: ${resultsContent?.substring(0, 200)}...`);
      }
    }
    ...
    if (stackButtonExists) {
      ...
      const stackResultsExpander = page.locator('summary:has-text("Stack Auth Test Results"), details:has-text("Stack Auth")').first();
      ...
      if (stackResultsVisible) {
        ...
        console.log(`   Stack auth results: ${stackContent?.substring(0, 200)}...`);
      }
    }

    // Take screenshot after debug tests
    await page.screenshot({ path: 'test-4-after-debug-tests.png', fullPage: true });

    // Step 7: Check Debug Panel
    console.log('\n7. Testing Debug Panel...');
    
    const debugPanelButton = page.locator('button:has-text("Show"), button:has-text("Hide")').first();
    ...
    const debugLogButton = page.locator('button:has-text("Debug Logs")').first();
    ...
    if (debugLogButtonExists) {
      ...
      const logViewer = page.locator('[data-testid="debug-log"], .debug-log').first();
      const logViewerVisible = await logViewer.isVisible();
      console.log(`   Debug log viewer visible: ${logViewerVisible}`);
      
      await page.screenshot({ path: 'test-6-debug-log-viewer.png', fullPage: true });
    }
    ...
    const welcomeMessage = page.locator('text=/Welcome.*enoteware/', 'h1:has-text("Welcome")').first();
    ...
    const scenarioCountText = page.locator('text=/Total Scenarios/', 'text=/0 scenarios/').first();
    ...
    const createScenarioButton = page.locator('button:has-text("Create"), a:has-text("Create")').first();
    ...
    if (createButtonExists) {
      ...
      await page.screenshot({ path: 'test-8-create-scenario.png', fullPage: true });
    }
    ...
    const userMenuButton = page.locator('button:has-text("enoteware"), [data-testid="user-menu"]').first();
    ...
    const navUserIndicator = page.locator('text=/enoteware/', 'text=/EN.*enoteware/').first();
