...
test.describe('HELOC Calculator E2E Test', () => {
  test('Complete HELOC calculator workflow with error monitoring', async ({ page, context }) => {
    ...
    try {
      ...
      const calculatorEmptyScreenshot = path.join(screenshotsDir, '02-calculator-empty.png');
      await page.screenshot({ path: calculatorEmptyScreenshot, fullPage: true });
      ...
      const fieldMappings = [
        { value: formData.homeValue, selectors: ['#homeValue', 'input[name="homeValue"]', 'input[placeholder*="home value" i]'] },
        { value: formData.mortgageBalance, selectors: ['#mortgageBalance', 'input[name="mortgageBalance"]', 'input[placeholder*="mortgage balance" i]'] },
        { value: formData.interestRate, selectors: ['#interestRate', 'input[name="interestRate"]', 'input[placeholder*="interest rate" i]'] },
        { value: formData.yearsRemaining, selectors: ['#yearsRemaining', 'input[name="yearsRemaining"]', 'input[placeholder*="years remaining" i]'] },
        { value: formData.monthlyPayment, selectors: ['#monthlyPayment', 'input[name="monthlyPayment"]', 'input[placeholder*="monthly payment" i]'] },
        { value: formData.monthlyIncome, selectors: ['#monthlyIncome', 'input[name="monthlyIncome"]', 'input[placeholder*="monthly income" i]'] },
        { value: formData.monthlyExpenses, selectors: ['#monthlyExpenses', 'input[name="monthlyExpenses"]', 'input[placeholder*="monthly expenses" i]'] },
        { value: formData.helocRate, selectors: ['#helocRate', 'input[name="helocRate"]', 'input[placeholder*="heloc.*rate" i]'] },
        { value: formData.helocLimit, selectors: ['#helocLimit', 'input[name="helocLimit"]', 'input[placeholder*="heloc.*limit" i]'] }
      ];
      ...
      for (const field of fieldMappings) {
        ...
        if (!filled) {
          testResults.uiIssues.push(`Could not fill field with value: ${field.value}`);
        }
      }

      // Take screenshot after filling form
      const calculatorFilledScreenshot = path.join(screenshotsDir, '03-calculator-filled.png');
      await page.screenshot({ path: calculatorFilledScreenshot, fullPage: true });
      ...
      if (!calculateClicked) {
        ...
        testResults.successfulSteps.push('Successfully clicked Calculate button');
      }
      ...
      const resultsScreenshot = path.join(screenshotsDir, '04-results-display.png');
      ...
      if (saveClicked) {
            ...
            if (await input.isVisible().catch(() => false)) {
              ...
              for (const confirmSelector of confirmSaveSelectors) {
                ...
                if (await confirmButton.isVisible().catch(() => false)) {
                  ...
                  testResults.successfulSteps.push('Successfully saved scenario');
                  break;
                }
              }
              break;
        ...
        const saveDialogScreenshot = path.join(screenshotsDir, '05-save-dialog.png');
      ...
      const scenariosScreenshot = path.join(screenshotsDir, '06-scenarios-list.png');
      ...
      const testScenarioVisible = await page.locator('text="Test Scenario"').isVisible().catch(() => false);
      if (testScenarioVisible) {
        testResults.successfulSteps.push('Test Scenario found in scenarios list');
      } else {
    ...
    testResults.consoleErrors.forEach(error => console.log('✗', JSON.stringify(error, null, 2)));
    
    console.log('\nNETWORK ERRORS:', testResults.networkErrors.length);
    testResults.networkErrors.forEach(error => console.log('✗', JSON.stringify(error, null, 2)));
    ...
    expect(testResults.consoleErrors.filter(e => e.type === 'error').length).toBe(0);
