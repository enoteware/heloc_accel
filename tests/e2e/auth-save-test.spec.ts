...
test.describe("Authenticated Save Functionality Test", () => {
  test("Complete authentication and save scenario flow", async ({ page }) => {
    ...
    await page.screenshot({
      path: "test-results/01-signin-page.png",
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: "test-results/03-calculator-after-login.png",
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: "test-results/04-demo-data-filled.png",
      fullPage: true,
    });
    ...
    await page.waitForSelector(
      '[data-testid="live-results"], .live-results, .results-section',
      { timeout: 10000 },
    );
    ...
    await page.screenshot({
      path: "test-results/05-live-results.png",
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: "test-results/06-save-modal.png",
      fullPage: true,
    });
    ...
    await page.fill(
      'input[name="name"], input[placeholder*="scenario name"], input[placeholder*="Name"]',
    ...
    await page.screenshot({
      path: "test-results/07-modal-filled.png",
      fullPage: true,
    });
    ...
    if (currentUrl.includes("/dashboard")) {
      ...
      await page.screenshot({
        path: "test-results/09-dashboard-success.png",
        fullPage: true,
      });
    } else {
      ...
      await page.screenshot({
        path: "test-results/09-no-redirect.png",
        fullPage: true,
      });
      ...
      await page.screenshot({
        path: "test-results/11-manual-dashboard-nav.png",
        fullPage: true,
      });
    }
    ...
    const successMessage = page.locator(
      '.alert-success, .success-message, .toast-success, [role="alert"]',
    );
    if ((await successMessage.count()) > 0) {
      console.log("✅ Success message found");
      await page.screenshot({
        path: "test-results/12-success-message.png",
        fullPage: true,
      });
    }
    ...
    const errorMessage = page.locator(
      '.alert-error, .error-message, .toast-error, [role="alert"][class*="error"]',
    );
    if ((await errorMessage.count()) > 0) {
      console.log("❌ Error message found");
      await page.screenshot({
        path: "test-results/12-error-message.png",
        fullPage: true,
      });
    }

    // Check for saved scenarios
    const scenarioItems = page.locator(
      '.scenario-item, .scenario-card, [data-testid*="scenario"]',
    );
    ...
    await page.screenshot({
      path: "test-results/13-final-dashboard.png",
      fullPage: true,
    });
    ...
    console.log(
      `Dashboard redirect: ${currentUrl.includes("/dashboard") ? "✅" : "❌"} ${currentUrl.includes("/dashboard") ? "Success" : "Failed"}`,
    );
    console.log(
      `Scenario saved: ${(await ourScenario.count()) > 0 ? "✅" : "❌"} ${(await ourScenario.count()) > 0 ? "Found" : "Not found"}`,
    );
