...
test.describe("Comprehensive Auth Testing", () => {
  test("complete authentication flow with debug testing", async ({ page }) => {
    ...
    const userInfoSection = page
      .locator(
        '[data-testid="user-info"], .user-info, h2:has-text("User Information")',
      )
      .first();
    ...
    if (await authDebugButton.isVisible()) {
      ...
      const authStatus = page
        .locator('[data-testid="auth-status"], .auth-status')
        .first();
    ...
    if (await stackAuthButton.isVisible()) {
      ...
      const stackAuthStatus = page
        .locator('[data-testid="stack-auth-status"], .stack-auth-status')
        .first();
    ...
    const scenarioElements = page.locator(
      '[data-testid="scenario"], .scenario-card, .scenario-item',
    );
    ...
    const debugLogButton = page
      .locator(
        'button:has-text("Debug Log Viewer"), button:has-text("Show Debug Log"), button:has-text("View Logs")',
      )
      .first();
    if (await debugLogButton.isVisible()) {
      ...
      const logContent = page
        .locator('[data-testid="debug-log"], .debug-log, .log-content')
        .first();
      if (await logContent.isVisible()) {
        const logText = await logContent.textContent();
        console.log(
          `Debug log content (first 500 chars): ${logText?.substring(0, 500)}...`,
        );
      }
    } else {
    ...
    const authIndicators = page.locator(
      '[data-testid="authenticated"], .authenticated, .user-menu, .logout-button',
    );
