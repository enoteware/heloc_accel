...
test.describe('Save Scenario Final Test', () => {
  test('Complete save flow with force click', async ({ page }) => {
    ...
    await page.fill('textarea[name="description"]', 'Testing save with proper auth');
    console.log('✅ Form filled');
    
    // Take screenshot before save
    await page.screenshot({ path: 'test-results/final-test-before-save.png', fullPage: true });
    
    // Step 5: Save with force click to bypass overlay
    console.log('Step 5: Saving scenario (force click)...');
    try {
      await page.locator('button:has-text("Save Scenario")').click({ force: true });
    ...
    await page.screenshot({ path: 'test-results/final-test-after-save.png', fullPage: true });
    ...
    if (currentUrl.includes('/dashboard')) {
      ...
      if (await scenarioElement.count() > 0) {
        ...
        await page.screenshot({ path: 'test-results/final-test-scenario-found.png', fullPage: true });
      } else {
        console.log('❌ Scenario not found on dashboard');
        
        // Check all scenario items
        const allScenarios = page.locator('[data-testid*="scenario"], .scenario-item, .scenario-card');
      ...
      console.log('❌ Not redirected to dashboard, trying manual navigation...');
      
      // Navigate manually to dashboard
      await page.goto('http://localhost:3000/en/dashboard');
      await page.waitForLoadState('networkidle');
      
      await page.screenshot({ path: 'test-results/final-test-manual-dashboard.png', fullPage: true });
      ...
      if (await scenarioElement.count() > 0) {
        ...
        console.log('❌ Scenario not found even on manually navigated dashboard');
        
        // Check localStorage for saved data
        const localStorageData = await page.evaluate(() => {
          const keys = Object.keys(localStorage);
          const helocKeys = keys.filter(key => key.includes('heloc') || key.includes('demo'));
          const result = {};
        ...
        console.log('LocalStorage HELOC data:', JSON.stringify(localStorageData, null, 2));
      }
    }
    ...
    console.log(`6. Dashboard redirect: ${currentUrl.includes('/dashboard') ? '✅ SUCCESS' : '❌ FAILED'}`);
    console.log('7. Check screenshots in test-results/ for visual confirmation');
  });
});