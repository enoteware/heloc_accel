...
test.describe('Stack Auth Demo Account Test', () => {
  ...
  test('Complete user account workflow', async ({ page }) => {
    ...
    await page.waitForURL(/\/(calculator|en\/calculator|$)/, { timeout: 15000 });
    ...
    await page.fill('input[name="monthlyExpenses"]', TEST_SCENARIO.monthlyExpenses);
    await page.fill('input[name="mortgageBalance"]', TEST_SCENARIO.mortgageBalance);
    await page.fill('input[name="mortgageRate"]', TEST_SCENARIO.mortgageRate);
    await page.fill('input[name="mortgagePayment"]', TEST_SCENARIO.mortgagePayment);
    ...
    await page.waitForSelector('.recharts-wrapper, .chart-container, [data-testid="results"]', { timeout: 10000 });
    
    // Save scenario
    await page.click('button:has-text("Save Scenario")');
    
    // Wait for modal
    await page.waitForSelector('input[placeholder*="scenario name"], input[name="scenarioName"], input[name="name"]');
    
    // Fill scenario name
    const nameInput = await page.locator('input[placeholder*="scenario name"], input[name="scenarioName"], input[name="name"]').first();
    ...
    const saveButton = await page.locator('button:has-text("Save"):not(:disabled), button:has-text("Create"):not(:disabled)').last();
    ...
    const scenarioCard = await page.locator(`text=${TEST_SCENARIO.name}`).first();
    ...
    const incomeValue = await page.locator('input[name="monthlyIncome"]').inputValue();
    ...
    // Recalculate
    await page.click('button:has-text("Calculate")');
    await page.waitForSelector('.recharts-wrapper, .chart-container, [data-testid="results"]');
    
    // Save updated scenario
    await page.click('button:has-text("Save Scenario")');
    
    // The name should already be filled
    const existingName = await page.locator('input[placeholder*="scenario name"], input[name="scenarioName"], input[name="name"]').first().inputValue();
    expect(existingName).toContain(TEST_SCENARIO.name);
    
    // Save
    await page.locator('button:has-text("Save"):not(:disabled), button:has-text("Update"):not(:disabled)').last().click();
