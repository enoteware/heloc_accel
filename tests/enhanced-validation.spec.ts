...
test.describe("Enhanced Form Validation", () => {
  ...
  test("should auto-calculate discretionary income", async ({ page }) => {
    ...
    const discretionaryInput = page.locator(
      'input[name="monthlyDiscretionaryIncome"]',
    );
  ...
  test("should show cross-field validation warnings", async ({ page }) => {
    ...
    await expect(
      page.locator("text=HELOC limit seems high relative to mortgage balance"),
    ).toBeVisible();
  });

  test("should update form progress as fields are completed", async ({
    page,
  }) => {
  ...
  test("should show validation hints for empty fields", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Enter your current mortgage principal balance"),
    ).toBeVisible();
    await expect(
      page.locator("text=Enter as percentage (e.g., 6.5 for 6.5%)"),
    ).toBeVisible();
  });

  test("should enable submit button only when form is valid", async ({
    page,
  }) => {
  ...
  test("should prefill demo data correctly", async ({ page }) => {
    ...
    await expect(
      page.locator('input[name="currentMortgageBalance"]'),
    ).toHaveValue("$350,000");
    await expect(page.locator('input[name="currentInterestRate"]')).toHaveValue(
      "6.5%",
    );
    await expect(page.locator('input[name="remainingTermMonths"]')).toHaveValue(
      "300",
    );
    await expect(page.locator('input[name="monthlyPayment"]')).toHaveValue(
      "$2,347",
    );
  ...
  test("should show contextual help tooltips", async ({ page }) => {
    ...
    const mortgageHelpButton = page.locator(
      'button[aria-label="Help for Current Mortgage Balance *"]',
    );
    ...
    await expect(
      page.locator("text=Enter the remaining principal balance"),
    ).toBeVisible();
  });

  test("should show different tooltip content for different fields", async ({
    page,
  }) => {
    // Test interest rate tooltip
    const rateHelpButton = page.locator(
      'button[aria-label="Help for Current Interest Rate *"]',
    );
    await rateHelpButton.hover();

    await expect(
      page.locator("text=Your mortgage's annual interest rate"),
    ).toBeVisible();
    ...
    const paymentHelpButton = page.locator(
      'button[aria-label="Help for Monthly Payment (P&I) *"]',
    );
    await paymentHelpButton.hover();

    await expect(
      page.locator("text=principal and interest payment only"),
    ).toBeVisible();
    await expect(
      page.locator("text=Do NOT include taxes, insurance"),
    ).toBeVisible();
  });
});
