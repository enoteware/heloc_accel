...
test.describe("Stack Auth Authentication Flows", () => {
  ...
  test("Sign-up flow with console error monitoring", async ({ page }) => {
    ...
    await page.screenshot({
      path: "tests/screenshots/signup-initial.png",
      fullPage: true,
    });

    // Wait for Stack Auth components to load
    await page.waitForSelector(
      '[data-testid="credential-sign-up"], input[type="email"], .stack-form',
      { timeout: 10000 },
    );
    ...
    if (await emailInput.isVisible()) {
      ...
      await page.screenshot({
        path: "tests/screenshots/signup-no-email-field.png",
        fullPage: true,
      });
    }
    ...
    if (await passwordInput.isVisible()) {
      ...
      await page.screenshot({
        path: "tests/screenshots/signup-no-password-field.png",
        fullPage: true,
      });
    }
    ...
    if (await submitButton.isVisible()) {
      ...
      await page.screenshot({
        path: "tests/screenshots/signup-before-submit.png",
        fullPage: true,
      });
      ...
      await page.screenshot({
        path: "tests/screenshots/signup-after-submit.png",
        fullPage: true,
      });
    } else {
      console.log("❌ Submit button not found");
      await page.screenshot({
        path: "tests/screenshots/signup-no-submit-button.png",
        fullPage: true,
      });
    }
  ...
  test("Sign-in flow with console error monitoring", async ({ page }) => {
    ...
    // Take screenshot of initial state
    await page.screenshot({
      path: "tests/screenshots/signin-initial.png",
      fullPage: true,
    });

    // Wait for Stack Auth components to load
    await page.waitForSelector(
      '[data-testid="credential-sign-in"], input[type="email"], .stack-form',
      { timeout: 10000 },
    );
    ...
    if (await signInButton.isVisible()) {
      await page.screenshot({
        path: "tests/screenshots/signin-before-submit.png",
        fullPage: true,
      });
      ...
      await page.waitForTimeout(3000);
      await page.screenshot({
        path: "tests/screenshots/signin-after-submit.png",
        fullPage: true,
      });
    }
  ...
  test("Stack Auth test page verification", async ({ page }) => {
    ...
    // Take screenshot
    await page.screenshot({
      path: "tests/screenshots/stack-auth-test.png",
      fullPage: true,
    });
    ...
    const hasSignInForm =
      (await page
        .locator(
          '[data-testid="credential-sign-in"], .stack-credential-sign-in',
        )
        .count()) > 0;
    const hasSignUpForm =
      (await page
        .locator(
          '[data-testid="credential-sign-up"], .stack-credential-sign-up',
        )
        .count()) > 0;
  ...
  test("Protected route access test", async ({ page }) => {
    ...
    await page.screenshot({
      path: "tests/screenshots/dashboard-unauth.png",
      fullPage: true,
    });
    ...
    const isRedirected =
      currentUrl.includes("/handler/sign-in") || currentUrl.includes("/login");
    const hasAuthRequired =
      (await page
        .locator("text*=sign in, text*=login, text*=authenticate")
        .count()) > 0;
