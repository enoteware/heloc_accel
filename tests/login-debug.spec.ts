...
test.describe("Login Authentication Debug", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on("requestfailed", (request) => {
      ...
      console.log(
        `Request failed: ${request.method()} ${request.url()} - ${failure.failure}`,
      );
    });
  ...
  test("Debug login with demo credentials", async ({ page }) => {
    ...
    if (!currentUrl.includes("/heloc")) {
      ...
      await page.goto("http://localhost:3000/heloc", {
        waitUntil: "networkidle",
      });
      await page.screenshot({
        path: "test-results/02-heloc-page.png",
        fullPage: true,
      });
    }
    ...
    await page.goto("http://localhost:3000/heloc/login", {
      waitUntil: "networkidle",
    });
    await page.screenshot({
      path: "test-results/03-login-page.png",
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: "test-results/05-after-submission.png",
      fullPage: true,
    });
    ...
    const errorMessages = await page
      .locator('.text-red-600, .bg-red-50, [role="alert"]')
      .allTextContents();
  ...
  test("Test auth API endpoints directly", async ({ page }) => {
    ...
    const response = await page.request.post(
      "http://localhost:3000/heloc/api/auth/signin/credentials",
      {
        data: {
    ...
    try {
      const simpleLoginResponse = await page.request.post(
        "http://localhost:3000/heloc/api/auth/simple-login",
        {
          data: {
            email: "demo@helocaccel.com",
            password: "DemoUser123!",
  ...
  test("Test authentication state and session", async ({ page }) => {
    ...
    console.log(
      "Post-login state:",
      JSON.stringify(postLoginAuthState, null, 2),
    );

    // Try to access a protected route
    await page.goto("http://localhost:3000/heloc/dashboard", {
      waitUntil: "networkidle",
    });
