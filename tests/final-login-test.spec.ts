...
test.describe('Final Login Test with Debug', () => {
  test('Test login with server debug logging', async ({ page }) => {
    ...
    if (finalUrl.includes('/dashboard')) {
      ...
      const userInfo = await page.locator('[data-testid="user-menu"], .user-menu, [aria-label*="user"]').textContent().catch(() => 'No user info found');
    ...
    if (dashboardUrl.includes('/dashboard')) {
      ...
      console.log('❌ Dashboard redirected to login - user is not authenticated');
    }
  });
  ...
  test('Verify demo user credentials are correct', async ({ page }) => {
    ...
    const demoInfo = await page.locator('text*="demo@helocaccel.com"').textContent().catch(() => null);
